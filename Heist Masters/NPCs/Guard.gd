...
func make_path():
	...
	path = navigation.get_simple_path(position, new_destination.position, false) #creates an array of points, which form the path
	

func navigate():
	var distance_to_destination = position.distance_to(path[0])
	if distance_to_destination > proximity_buffer:
		move()
	else:
		update_path()
